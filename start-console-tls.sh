...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        echo "${FLAG}"
        podman run --pull always -v ./console-cert:/console-cert --rm --network=host --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
    else
        ...
        echo "running podman with opts:"
        echo "${FLAG}"
        podman run --pull always -v ./console-cert:/console-cert --rm -p "$CONSOLE_PORT":9442 --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
    fi
else
    ...
    echo "running podman with opts:"
    echo "${FLAG}"
    docker run --pull always -v ./console-cert:/console-cert --rm -p "$CONSOLE_PORT":9442 --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
fi
